...
// Em Objects a chave sÃ³ pode ser string ou symbol (number Ã© coergido a string)
...
// if() = coerÃ§ao implicita para boolean e retorna false
// O jeito certo em Object Ã© ({ name: 'Erick' }).hasOwnProperty('name')
assert.ok(myMap.has(onlyRefenceWorks))

// para remover um item do objeto
//  delete item.id 
// imperformÃ¡tico para o Javascript
assert.ok(myMap.delete(onlyRefenceWorks))

// Nao dÃ¡ para iterar em Objects diretamente
...
// Pode ser coletado apÃ³s perder as referencias
// usado em casos beeem especÃ­ficos

// tem a maioria dos beneficos do Map
// MAS: nao Ã© iterÃ¡vel
//  SÃ³ chaves de referencia e que vocÃª jÃ¡ conheÃ§a
// mais leve e preve leak de memoria, pq depois que as instancias saem da memoria, tudo Ã© limpo
